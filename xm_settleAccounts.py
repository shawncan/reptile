...
class Settlement(object):
    ...
    def readCaptain(self):
        ...
        for row in readSheet.rows:
            ...
            self.captainList.append(head)

        self.captainList = list(set(self.captainList))
    ...
    def dataProcessing(self):
        ...
        for captain in self.captainList:
            ...
            for infoData in self.excelInfoList:
                ...
                if head == captain:
                    headDataList.append(infoData)

            self.billingData[captain] = headDataList
    ...
    def readQuantity(self):
        ...
        for infoList in self.billingData:
            for infoData in self.billingData[infoList]:
                account = infoData["账号"]
    ...
    def exportExcel(self):
        for captain in self.captainList:
            outputName = captain + '.xlsx'
            workbook = openpyxl.Workbook()
            workbook.save(outputName)


        for infoList in self.billingData:
            outputName = infoList + '.xlsx'
            ...
            for i in range(len(self.billingData[infoList])):

                applicationTime = self.billingData[infoList][i]['申请时间']
                user = self.billingData[infoList][i]['标注人']
                head = self.billingData[infoList][i]['团队长']
                details = self.billingData[infoList][i]['账号']
                correctRate = self.billingData[infoList][i]['正确率']
                quantity = self.billingData[infoList][i]['数量']
                calculateCost = self.billingData[infoList][i]['价格']
